...
def optimize_routes(
    ...
    if len(eligible_bins) < before_count:
        logger.warning(f"[RouteOptimizer] WARNING: Removed {before_count - len(eligible_bins)} bins with invalid coordinates")

    # ── Diagnostic logging ──────────────────────────────────────
    logger.info(f"[RouteOptimizer] Total bins in zone: {len(bins_data)}")
    logger.info(f"[RouteOptimizer] Collection threshold: {threshold}%")
    logger.info(f"[RouteOptimizer] Truck capacity: {truck_capacity_kg} kg")
    logger.info(f"[RouteOptimizer] Eligible bins: {len(eligible_bins)}")
    for b in eligible_bins:
        logger.debug(f"  → Bin {b.get('id')} ({b.get('label')}): fill={b.get('fill_level')}%, status={b.get('status')}")
    ...
    logger.info(f"[RouteOptimizer] Total estimated demand: {total_demand} kg vs capacity: {int(truck_capacity_kg)} kg")
    ...
    if not solution:
        logger.error(f"[RouteOptimizer] VRP solver found NO solution — using fallback sort")
    ...
    logger.info(f"[RouteOptimizer] VRP solution: {len(route_order)}/{len(eligible_bins)} bins included, {dropped_count} dropped")
    if dropped_count > 0:
        included_ids = {b["id"] for b in route_order}
        for b in eligible_bins:
            if b["id"] not in included_ids:
                logger.warning(f"  ⚠ DROPPED: Bin {b['id']} ({b['label']}), fill={b['fill_level']}%, status={b['status']}")
